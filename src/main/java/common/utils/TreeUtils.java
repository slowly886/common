...
public class TreeUtils {
    ...
    //遍历获取下面子节点的count
    public static <T> T countCtree(T list,String childrenTreeName,String countfieldName){
        List<T> countCtrees = (List<T>) ReflectUtils.getValueByField(childrenTreeName, list);
        if(countCtrees==null){
            return list;
        }
        int size = countCtrees.size();
        if(size>0){
            for (T countCtree : countCtrees) {
                ReflectUtils.setValueByField(countfieldName,list,size);
                countCtree(countCtree,childrenTreeName,countfieldName);
            }
        }
       /* for (T t : list) {
        Object count = ReflectUtils.getValueByField("count", t);
